...
async function uploadImage(
  ...
  try {
    ...
    const resData = responseSchema.parse(await res.json());
...
async function requestClientScreenshot(
  ...
  const playerSrcSchema = z.union([
    z.string().min(1, "Player source must be a non-empty string"),
    z.number(),
  ]);
  playerSrcSchema.parse(playerSrc);
